...
\begin{itemize}
	\item \textbf{Guided Hints:} Standard ControlNet injects conditioning into a single UNet decoder head, which is used to condition on the control maps. As MESA has two separate Heads it raised the question of whether one shared hint suffices or whether separate hints per head yield better feature alignment.
...
The qualitative comparison of generated satellite imagery across different control weights $w$ and conditioning maps is shown in Figure~\ref{fig:qualitative_comparison}. Each row corresponds to a different validation feature map (Sample~1--4), and each column shows the result of a different conditioning scale ($w = 0.0, 0.5, 1.0, 1.5, 2.0$). The leftmost column displays the control map used as input to the ControlNet, where channels encode valleys (R), ridges (G), and cliffs (B). At $w = 0.0$, the model receives no conditioning signal and produces purely text-guided outputs, which exhibit a high degree of visual quality but lack alignment with the intended terrain features. However, as $w$ increases, there is a noticeable decrease in visual quality, with higher weights leading to the overall image becoming flatter and less detailed.
