...
\begin{minipage}{\textwidth}
	To investigate this hypothesis, the training data distribution and the distribution of samples generated by the model were compared. For easier comparison, both the generated data and the training data were mapped from $[-1,~1]$ to $[0,~1]$. The results indicate that the normalized DEM distribution differs from the distribution the pretrained model was originally exposed to. The generated samples are centered around 0.5, while the training data centers around 0.0 for RGB samples. For DEM samples, the center of the distribution is more similar, with 0.4 for training data and 0.5 for generated samples. However, the training data distribution is much more spread out across the whole range from 0 to 1, while the generated samples show a much narrower distribution around the center. This suggests that the model is struggling to learn the new data distribution, which may be contributing to the degradation of the outputs.
\end{minipage}
...
The qualitative comparison for the second training run, shown in Figures~\ref{fig:new_qualitative_comparison} and~\ref{fig:new_qualitative_comparison_dem}, demonstrates the effect of varying the conditioning scale $w$ on the generated terrain using the model trained with the new normalization strategy. Compared to the first training run, the generated images maintain better visual quality at higher control weights, with fewer artifacts than previously observed. The model broadly follows the conditioning maps, and higher control weights lead to better alignment with the intended terrain features. However, there is still a noticeable decrease in visual quality at higher control weights, suggesting that a better balance between conditioning strength and visual quality is still needed. Features other than valleys still show relatively low alignment with the conditioning maps, which is consistent with the quantitative evaluation results.
...
The quantitative evaluation of the second training run, shown in Figure~\ref{fig:new_iou_distribution}, presents the per-channel IoU distribution across 100 generated terrain samples with a control weight of $w = 1.4$. The results show improved mean IoU for ridges and cliffs compared to the first training run, but mean IoU for valleys has decreased on average. The variance of IoU scores remains high for all channels, which may still reflect undertraining or the sensitivity of the IoU metric to small misalignments in the feature maps. Overall, while there is some improvement in alignment with the conditioning maps, there is still room for further optimization and training to achieve more consistent results across all terrain features.
...
% Fix for akward page split
\begin{minipage}{\textwidth}
	To further evaluate the model's ability to learn spatial conditioning, a set of synthetic feature maps with simple geometric patterns was created and used as conditioning inputs for generation. The results, shown in Figure~\ref{fig:synthetic_feature_maps}, demonstrate that the model is able to generate landscapes that generally follow the structure of the synthetic conditioning maps, with the generated DEMs and satellite images showing alignment with the intended patterns. However, in some cases the conditioning has little impact, especially on images with only ridges or cliffs, which is consistent with the quantitative evaluation results showing lower IoU for these features. When the feature map is too far removed from plausible real-world terrain, the model struggles to generate meaningful outputs.
\end{minipage}
\begin{figure}[htbp]
	\centering
